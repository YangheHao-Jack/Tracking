...
max_corners = 2500
quality_level = 0.01  # Higher quality level for stronger features
min_distance = 10
...
while True:
    ...
    # If no points are detected, fallback to the Kalman-predicted position
    if feature_points is None:
        print("No feature points detected.")
        predicted = kalman.predict()
        detected_tip = (int(predicted[0]), int(predicted[1]))
    else:
        print(f"Number of feature points detected: {len(feature_points)}")
        
        # Convert feature points to a list of tuples for easier manipulation
        points = [tuple(point.ravel()) for point in feature_points]

        # Find the point farthest to the left (minimum x-coordinate)
        detected_tip = min(points, key=lambda p: p[0])

        # Draw all feature points for visual confirmation
        for point in points:
            cv2.circle(frame, (int(point[0]), int(point[1])), 3, (0, 255, 255), -1)  # Yellow dots for detected feature points

    # Kalman filter correction if a new detection is found
    if detected_tip is not None:
        # Ensure the detected_tip is in integer format
        detected_tip = (int(detected_tip[0]), int(detected_tip[1]))
        ...
        # Use the Kalman-predicted position if no tip is detected nearby
        predicted = kalman.predict()
