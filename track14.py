...
def get_trackbar_settings(window_name):
    settings = {
        ...
        "SVM C": cv2.getTrackbarPos("SVM C", window_name),
    }
    return settings
...
cv2.createTrackbar("SVM C", trackbar_window, 1, 100, lambda x: None)
...
while True:
    ...
    if feature_points is not None:
        ...
        # Additional outlier filtering using SVM
        if len(points) > 2:
            scaler = StandardScaler()
            scaled_points = scaler.fit_transform(points_array)
            svm = SVC(kernel='rbf', gamma=settings["SVM Gamma"], C=settings["SVM C"])
            labels = svm.fit_predict(scaled_points)
